...
class DataValidateExt{
     ...
     * 
     * @param $aryData array, source $_GET/$_POST/input_array 
     * Example:
     *  $aryData = array(
     *      'name'  => 'Zachary',
     *      'email' => 'hezachary@gmail.com',
     *      'password' => '123456',
     ...
     * 
     **/
    public static function validate($aryData, $arySettingList, $aryExtraParams = null){
        $aryReturn = array();
        $aryMsg = array();
        foreach($arySettingList as $fieldName => $arySubSettingList){
            ...
            foreach($arySubSettingList as $key => $arySetting){
                ...
                $tmpData = false;
                if( array_key_exists( 'filter', $arySetting ) && $arySetting['filter']){
                    $tmpData = filter_var($aryReturn[$fieldName], $arySetting['filter'], $arySetting);
                }elseif(array_key_exists( 'call_func', $arySetting ) && $arySetting['call_func']){
                    $tmpData = call_user_func_array($arySetting['call_func'], array($aryReturn[$fieldName], array_key_exists( 'options', $arySetting ) ? $arySetting['options'] : '', $aryData, $aryExtraParams));
                }else{
                    $tmpData = self::validateField($aryReturn[$fieldName], $arySetting['options'], $aryData);
                }
                
                if($tmpData === false){
                    $strMsg = array_key_exists( 'msg', $arySetting ) && $arySetting['msg'] ? $arySetting['msg'] : $arySubSettingList['msg'];
    ...
    protected static function uniquEmail($strFieldValueOrg, $strFieldSettings = null, $aryFieldList = array()){
        return $strFieldValueOrg == 'hezachary@gmail.com' ? false : $strFieldValueOrg;
    }
    
    /**
     * Validate Field
     * @param $strFieldValueOrg string  
     * Base format:
     * strMode::expression
     * -- strMode
     ...
     * @return value/false
     **/
    public static function validateField($strFieldValueOrg, $strFieldSettings, $aryFieldList = array()){
        ...
        switch($strMode){
            ...
            case 'text':
                ...
                break;
            case 'words':
                $strFieldValueOrg = preg_replace( '/(\s)+/' , '$1', trim(htmlentities(strip_tags($strFieldValueOrg), ENT_COMPAT, 'UTF-8')));
                $strFieldValue = self::rangeValidation( sizeof( explode( " ", preg_replace( '/\s+/' , ' ', $strFieldValueOrg ) ) ) , $strValidateFieldSettings) ? $strFieldValueOrg :  false;
                break;
            case 'pair':
